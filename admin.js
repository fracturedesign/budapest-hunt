...
(function () {
  ...
  var loadedFromEmptyFallback = false;
  ...
  function loadDraft() {
    ...
    if (typeof HUNT !== "undefined") return clone(HUNT);

    // content.js is encrypted (config.accessCode was set on whatever export
    // is currently deployed) and there's no local draft to fall back to —
    // this device has never edited this hunt before. Decrypting here would
    // mean making this whole file's boot sequence async just for a rare
    // cold-start case; instead, start from an empty hunt and point at the
    // real recovery path: a .json backup, which every export already
    // encourages ("Backup .json" sits right next to "Export content.js").
    loadedFromEmptyFallback = true;
    return {
      config: {
        groomName: "", title: "New Hunt", subtitle: "", estimatedDuration: "", estimatedDistance: "",
        howToPlay: [], hintPenaltyMinutes: 5, skipAfterWrongAnswers: 4, skipPenaltyMinutes: 15,
        wrongAnswerMessages: [], correctAnswerMessages: [], finishTitle: "", finishBlurb: []
      },
      stops: []
    };
  }
  ...
  function renderSettings(main) {
    ...
    main.appendChild(el("fieldset", { class: "fieldset" },
      ...
      field("Code", textInput(c.accessCode, set("accessCode")),
            "Leave blank for no gate at all (the default — \"Export content.js\" produces a normal, readable file, anyone " +
            "with the link opens straight to the start screen). Set one and \"Export content.js\" instead encrypts the " +
            "WHOLE hunt with it — puzzles, answers, photos, everything — so a viewer without the code sees only " +
            "ciphertext, not just a hidden check. The group has to type it in before they see anything else, once per " +
            "browser — they're never asked again on that phone until the hunt is reset back to the beginning."),
      el("div", { class: "warnbox info" },
        "There is NO recovery if this code is lost. An encrypted export is permanently unreadable without it — not by " +
        "you, not by anyone. Your draft here in the browser (and any \".json\" backup) is completely unaffected either " +
        "way; this only changes what \"Export content.js\" produces. Write the code down somewhere safe."
      )
    ));
  ...
  function contentHeader(extraNote) {
    return (
      "/* ============================================================================\n" +
  ...
  function exportContentJs() {
    ...
    var code = data.config && data.config.accessCode && String(data.config.accessCode).trim();
    if (code) { exportEncryptedContentJs(data, code); return; }
    ...
    download("content.js", contentHeader() + body, "text/javascript;charset=utf-8");
    setSaveBar("Exported content.js — replace the old file with it, then reload the game.", false);
    toast("content.js downloaded");
  }

  /**
   * The whole hunt, encrypted with the access code as the passphrase (see
   * HuntLogic.encryptWithCode — AES-256-GCM, keyed via PBKDF2). Whoever
   * opens the deployed site sees only ciphertext until they enter the
   * code; see bootLocked() in app.js for the other half of this.
   *
   * The code itself is deliberately never written into the output, not
   * even inside the encrypted blob — it only ever exists as the
   * passphrase used to derive the key, both here and when the game
   * decrypts it. There is no recovery if it's lost: this export becomes
   * permanently unreadable ciphertext without it. Your local draft (and
   * any .json backup) is unaffected — this only touches what gets
   * downloaded as content.js.
   */
  async function exportEncryptedContentJs(data, code) {
    setSaveBar("Encrypting content.js…", false);

    // These 5 labels ship in the clear alongside the ciphertext — they're
    // the lock screen's OWN copy, needed before there's anything to
    // decrypt yet, and not remotely sensitive (just "enter the code"
    // chrome, teaches an onlooker nothing about the hunt itself).
    var resolved = HuntLogic.resolveLabels(data.config);
    var lockLabels = {
      lockKicker: resolved.lockKicker,
      lockTitle: resolved.lockTitle,
      lockPlaceholder: resolved.lockPlaceholder,
      unlockButton: resolved.unlockButton,
      lockWrongCode: resolved.lockWrongCode
    };

    var payload = clone(data);
    delete payload.config.accessCode;

    var blob;
    try {
      blob = await HuntLogic.encryptWithCode(JSON.stringify(payload), code);
    } catch (e) {
      alert("Couldn't encrypt this export: " + e.message);
    ...
    var note =
      " * ENCRYPTED — an access code is set, so this file is ciphertext, not the\n" +
      " * real hunt data. It can only be read by whoever enters the correct code\n" +
      " * on the lock screen. If the code is ever lost, this specific export is\n" +
      " * permanently unreadable — there's no recovery built in. Your editing\n" +
      " * draft in this browser (and any .json backup) is unaffected.\n";

    var body = "var HUNT_ENCRYPTED = " + JSON.stringify({
      v: 1,
      salt: blob.salt,
      iv: blob.iv,
      ...
      "/* Not decryptable without the code — the Node test harness can't read\n" +
      " * this file directly. See README.md for how to validate an encrypted\n" +
      " * export before deploying it. */\n" +
      "if (typeof module !== \"undefined\" && module.exports) {\n" +
      "  module.exports = { encrypted: true, v: 1, salt: HUNT_ENCRYPTED.salt,\n" +
      "                      iv: HUNT_ENCRYPTED.iv, ciphertext: HUNT_ENCRYPTED.ciphertext };\n" +
      "}\n";

    download("content.js", contentHeader(note) + body, "text/javascript;charset=utf-8");
    setSaveBar("Exported an ENCRYPTED content.js — the code is required to open it, with no recovery if it's lost.", false);
    toast("Encrypted content.js downloaded");
  }
  ...
  setSaveBar(loadedFromEmptyFallback
    ? "⚠️ content.js is encrypted and this browser has no draft for it — starting from an empty hunt. " +
      "If you have a .json backup, use \"Restore .json\" instead of building from scratch."
    : hasStoredDraft
    ? "Editing an unsaved draft from this browser · export content.js to make it live"
    : "Loaded from content.js · edits autosave here as a draft", hasStoredDraft || loadedFromEmptyFallback);
