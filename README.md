...
Nine tasks, real landmarks, a walkable loop that ends in the ruin bar district.
...
To update content later: edit `content.js`, then `git commit -am "fix task 4"
...
| | |
|---|---|
| **Edit any task** | Name, teaser, travel clue (optional — can be skipped for on-site puzzles), arrival note, puzzle text, hints, accepted answers, success message |
...
| **Add / delete / duplicate tasks** | `+ Add` in the sidebar, or the `⧉` and `✕` buttons on each row |
| **Reorder** | `↑` `↓` on each task. Numbering, the progress bar and "Task N of M" all follow automatically |
...
| **Scoring & STAG levels** | Per-task timer (independently toggleable from scoring), bonus/decay points, hint penalties, and the title earned at the end — see below |
| **Photo capture** | An in-app "take a photo" button, on by default for Dare tasks — see below |
...
| **Organiser notes** | A per-task notes field that players never see. The route-verification notes already live here |
| **Pre-event checklist** | Every task that's still missing something, plus your notes, in one list |
...
Each task picks one. They change what players actually do, not just the wording.
...
| **Multiple choice** | The options as big tap-targets | Good when the answer is hard to spell, or for an "odd one out". The editor errors if none of your options matches an accepted answer, so you can't ship an unwinnable task |
...
| **Info only** | One continue button | No challenge at all. A story beat, a warning, a waypoint. No "correct!" screen either — the continue button goes straight to the next task |
| **Picker (choose-your-path)** | Several labelled buttons, each with an optional description, colour and a small shimmer animation | Not a quiz — there's no right answer, just a fork. Tapping one jumps straight to that option's target task by id. Whichever option(s) weren't tapped are excluded from the rest of that playthrough entirely: never played, not shown in the finish-screen recap, and not counted in the STAG score's possible-points total. The editor requires at least two options, each with a label and a valid target task, and flags a target that's been renamed/deleted out from under it |
| **One-try choice** | The options as big tap-targets, same as Multiple choice | No retry: the *first* tap is the only one that counts. Get it right and it behaves exactly like Multiple choice (success screen, points). Get it wrong and it's an outright fail — no shake-and-try-again, no hints escape hatch (there's nothing left to escape) — straight to its own failure screen (own emoji, headline, message and photo, all overridable per task), then on to the next task like normal. Scores 0 on a fail, same treatment as a skip, and shows as "failed 💀" in the finish-screen recap rather than "skipped" or "solved" |
| **Timed sequence** | A sub-task's label, instructions and optional photo, plus a big countdown and a skip button | Several timed sub-tasks in a row (e.g. "Russian accent for 15 minutes, then British, then pirate"), each with its own custom duration. Mostly passive — nothing to answer — but each sub-task also has its own skip button (custom text per sub-task, optional) for a group that wants to move on early. A sub-task's clock hitting 0 (or its skip button being tapped) auto-advances to the next one; finishing the last one solves the task like any other type (success screen, points if you've turned scoring on). Skipping any sub-task except the last one costs points ("Points lost per skipped sub-task", default 50); skipping the LAST one instead fails the whole task outright — straight to a failure screen (shared with One-try choice's), 0 points, shown as "failed 💀" in the recap. Unscored by default, like Info only — it's a fixed-duration activity, not a race. Wall-clock based like every other timer here, so a reload mid-sequence resumes at the right sub-task with the right time remaining, even catching up through any that fully expired while the tab was closed. The editor requires at least one sub-task, each with a label, a positive duration, and (nudged, not required) some instructions |
...
specific task is still your call — see "Include in scoring" below — that's a
deliberate choice made once while writing that task, not something that can
silently drift.

**How it scores a task:**

- The clock for a task starts the instant the group taps **WE'RE HERE** —
...
- **Skipping** a task always scores 0 — the one outcome worse than just being
  slow.

Every number is a per-task override of a game-wide default, exactly like the
label system: leave a task's box blank and it inherits **Scoring & STAG
...
Not every task has to count. **Info** tasks (pure story beats/waypoints)
are excluded by default since there's nothing to judge; flip "Include in
scoring" on any task to override either direction — worth doing, for
instance, on task 9's sincere finale, if racing a clock feels wrong for that
one.
...
On the puzzle screen, a live ⏱ clock shows next to each scored task (turning
...
knows the points/STAG system exists before they hit the first task, not just
...
switches.** Each task's "Scoring & timer" section has its own "Show a live
timer" toggle, independent of "Include in scoring" — so you can put a
countdown on an info task purely for pace with no points attached, or hide
the clock on a task that still counts, whatever reads better for that
particular moment.

### Photo capture

Any task can offer an in-app **"📸 Take a photo"** button — on by default for
**Dare** tasks, off for everything else, overridable per task regardless of
type (**Photo capture** section on each task). Tapping it opens the phone's
...
> attach it to the task. Belt and braces.
...
When it's on, one extra screen appears right after the last task, before the
results: everyone piles in for a group selfie, using the exact same
capture/fallback/save-to-Photos pipeline as a task's photo control. It's
deliberately **not** a task — no timer, no points, not counted in "Task N of
M" — and the photo is required; there's no skipping past it (mirrors the
per-task required-photo gate, including the same keyboard-bypass guard).

On the finish screen this pairs side-by-side with the **earliest** photo
taken anywhere earlier in the hunt (first match in task order) under a
...
Some labels take tokens: `{n}` (hint number / current task), `{total}` (number
of tasks), `{min}` (the relevant penalty), `{skipMin}` (skip penalty),
`{pts}`/`{earned}`/`{possible}`/`{percent}` (the points system, see below).
For example `"Leg {n} of {total}"` or `"Peek at hint {n} (costs {min}m)"`.

**Per-task overrides** live on the task itself, and beat the game-wide label:
...
success headline. Handy for making one task read differently — e.g. `🎯 The
...
in it are lost. Put notes in a task's **Organiser notes** field instead — that's
...
reference, five tasks ship with `[INSERT: ...]` placeholders:

| Task | What it is | What you need to write |
|------|-----------|------------------------|
...
| **9** — Last Task | Personal, **sincere** | The earnest one. Write this last, write it properly. |

Tasks 3 and 9 also have `"[ANSWER: fill in later]"` in their `answers` list.

**Placeholder tasks are playable as-is.** Any answer string starting with
`[ANSWER:` tells the app the task isn't written yet: it shows an amber warning
...
skipPenaltyMinutes: 15,      // cost of skipping a task entirely
```

The "give up" button only appears once the group has revealed **every** hint on
that task **and** been wrong `skipAfterWrongAnswers` times — so it can't be
stumbled into, but nobody's night ends stuck on a puzzle because a statue moved.

### "Skip this task — no penalty"

A second, separate skip control, always visible at the bottom of every task
...
doing that specific task. Tapping it (after a confirmation prompt, so it can't
be fat-fingered mid-task) jumps straight to the next task and costs **nothing**
— no time penalty, no points lost. The task is excluded from that playthrough
entirely, the same way an untaken picker branch is: it drops out of "Task N of
...
"Buttons & labels" (`jumpSkipButton`); there's no per-task override since it's
meant to behave identically everywhere.

### Adding, removing or reordering tasks

Add or delete objects in the `stops` array (the underlying field name is unchanged). The progress counter, "Task N of M"
...
structure of every task in
`content.js`. It also prints which tasks still contain unfilled placeholders.
...
- [ ] **Walk the route.** Every task has an **Organiser notes** entry naming the
...
- [ ] **Count the Gozsdu udvar courtyards yourself** (task 8). The answer
...
- [ ] **Pick the two bars** and put their names in tasks 7 and 8.
- [ ] **Fill in the three Danil tasks** (3, 5, 9).
...
Each task uses a different puzzle mechanic on purpose. The drinking challenges
at tasks 7–9 are tied to things you can *observe* at the bar, not to actually
...
monuments on Szabadság tér. Both are a short detour from task 4 and worth
...
current task, which view they're on, hints revealed, wrong-answer counts,
accepted answers, skipped tasks, and the start timestamp.
...
- **No undo in the editor.** Deleting a task asks for confirmation and that's
...
- **Recurrence of "not quite" messages** cycles through a fixed list, so on a
  very bad task the group will see one repeat. Add more strings to
