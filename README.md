...
| **Timed sequence** | A sub-task's label, instructions and optional photo, plus a big countdown and a skip button | Several timed sub-tasks in a row (e.g. "Russian accent for 15 minutes, then British, then pirate"), each with its own custom duration. Mostly passive — nothing to answer — but each sub-task also has its own skip button (custom text per sub-task, optional) for a group that wants to move on early. A sub-task's clock hitting 0 (or its skip button being tapped) auto-advances to the next one; finishing the last one solves the stop like any other type (success screen, points if you've turned scoring on). Skipping any sub-task except the last one costs points ("Points lost per skipped sub-task", default 50); skipping the LAST one instead fails the whole stop outright — straight to a failure screen (shared with One-try choice's), 0 points, shown as "failed 💀" in the recap. Unscored by default, like Info only — it's a fixed-duration activity, not a race. Wall-clock based like every other timer here, so a reload mid-sequence resumes at the right sub-task with the right time remaining, even catching up through any that fully expired while the tab was closed. The editor requires at least one sub-task, each with a label, a positive duration, and (nudged, not required) some instructions |
